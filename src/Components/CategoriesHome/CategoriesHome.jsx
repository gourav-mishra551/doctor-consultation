...
function CategoriesHome() {
    ...
    const updateScreenSize = () => {
        ...
        if (width <= 640) { // Small screen (mobile)
            setTotalCount(4); // Show 4 items
        } else if (width <= 1024) { // Tablet screen
    ...
    const FetchCategory = async () => {
        try {
            const res = await axios.get(`https://api.assetorix.com/ah/api/v1/dc/user/Category?limit=${totalCount}`);
