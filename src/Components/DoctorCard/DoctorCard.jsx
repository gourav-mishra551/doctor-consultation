...
const DoctorCard = ({ doctorData }) => {
    ...
    const calculateMinPrice = (doctors) => {
        ...
        doctors.forEach((doctor) => {
            if (doctor.slots) {
                if (doctor.slots.visitingMode === "online" || doctor.slots.visitingMode === "both") {
                    const availableOnlineSlots = doctor.slots.onlineSlots?.filter(
                        (slot) => !slot.isBooked && typeof slot.doctorCharge === "number" && !isNaN(slot.doctorCharge)
                    );

                    if (availableOnlineSlots?.length > 0) {
                        minPrice = Math.min(minPrice, ...availableOnlineSlots.map((slot) => slot.doctorCharge));
                    }
                }

                if (doctor.slots.visitingMode === "offline" || doctor.slots.visitingMode === "both") {
                    const availableOfflineSlots = doctor.slots.offlineSlots?.filter(
                        (slot) => !slot.isBooked && typeof slot.doctorCharge === "number" && !isNaN(slot.doctorCharge)
                    );

                    if (availableOfflineSlots?.length > 0) {
                        minPrice = Math.min(minPrice, ...availableOfflineSlots.map((slot) => slot.doctorCharge));
                    }
                }
            }
        });
                ...
                >
                    <div>
                            ...
                            <div className="flex-1 text-center lg:text-left">
                                <div className='flex  justify-between border-b pb-2 flex-row md:justify-between items-center lg:items-start'>
                                    <div className='flex flex-col md:flex-row md:gap-4 items-center'>
                                        <h2 className="text-lg md:text-xl lg:text-2xl font-bold text-[#00768A] capitalize">Dr. {doctor.userData.name}</h2>
                                        <p className="text-md text-sm md:text-lg text-[#8B4513] font-semibold">Experience: {doctor?.totalExperience} years</p>
                                    </div>
                                    ...
                                    <div className="md:w-auto w-[40%] lg:flex flex-col items-center mt-3 lg:mt-0">
                                        ...
                                        <div className="text-sm font-semibold text-gray-700">
                                            ...
                                            <span className="ml-2 text-green-600">
                                                {minPrice !== null ? `INR ${minPrice}` : 'Not available'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-2 flex-wrap mt-2 justify-center lg:justify-start">
                                    {doctor.services_offered.map((service, idx) => (
                                        <span key={idx} className="bg-[#f0f9f9] px-3 py-1 rounded-lg text-[#00768A] font-medium text-sm">
                                            {service}
                                        </span>
                                    ))}
                                </div>
                                    ...
                                    {doctor?.years_of_experience?.map((experience, idx) => (
                                        <React.Fragment key={idx}>
                                            {experience.skills.map((skill, skillIdx) => (
                                                <span key={skillIdx} className="text-sm text-gray-600 font-semibold">
                                                    {skill}
                                                </span>
                                            ))}
                                        </React.Fragment>
                        ...
                        <div className="flex flex-col md:flex-row gap-3 mt-4 w-full lg:w-auto">
                            ...
                            <button type="button" class="text-white md:w-[160px] w-full font-semibold bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-300 dark:focus:ring-teal-800  rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 shadow-md">Book</button>
                        </div>
                    </div>
                </div>
            ))
