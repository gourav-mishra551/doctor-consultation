...
function DrAppointmentCreation() {
  ...
  const handleEditSlotAndPrice = (index, isOnline) => {
    ...
    setNewSlotValue(
      isOnline ? data.onlineSlots[index] : data.offlineSlots[index]
    ); // Set the value of the slot being edited
  ...
  const saveSlotAndPrice = async () => {
    setData((prev) => {
      const updatedSlots = isEditingOnline
        ? [...prev.onlineSlots]
        : [...prev.offlineSlots];
  ...
  const generateSlots = () => {
    ...
    if (startTime >= endTime) {
      setError(
        "Please select a valid time. 'From' time cannot be greater than 'To' time."
      );
    ...
    while (currentTime < endTime) {
      ...
      const slotStart = currentTime.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      });
      const slotEnd = nextSlot.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      });
  ...
  return (
    <div className="sm:w-[650px] mx-auto p-5 border rounded-lg shadow-lg bg-white">
      ...
      <form onSubmit={HandleGeneratedSlots}>
        ...
        <div className="mb-[20px]" style={{ color: "red" }}>
        ...
        <div className="mb-4 flex justify-between">
          ...
          <div>
            ...
            data.onlineSlots.length === 0 ? null : (
              <>
                <label className="block text-gray-700 font-semibold mb-2">
                  Price
                </label>
                ...
                {data.offlineSlots.map((slot, index) => (
                  ...
                  >
                    ...
                    isEditingPriceSlot ? (
                      ...
                      <>
                        <span>
                          {`Start Time: ${slot.startTime || "N/A"}, End Time: ${
                            slot.endTime || "N/A"
                          }, Charge: ${slot.doctorCharge || 0}`}
                        </span>
                        {generatedPrice && (
                          <div
                            className="mt-0 p-2 rounded-[5px]"
                        ...
                        <div className="flex">
                          <CiEdit
                            ...
                            onClick={() => handleEditSlotAndPrice(index, false)}
                          />
                          <MdDelete
                            className="text-xl cursor-pointer text-red-500"
                            onClick={() => handleDeleteSlot(index, false)}
                          />
                        </div>
                      </>
                    )}
                    ...
                    isEditingPriceSlot ? (
                      ...
                      <>
                        ...
                        <div className="flex">
                          <CiEdit
                            className="mr-2 cursor-pointer text-xl text-blue-500"
                            onClick={() => handleEditSlotAndPrice(index, true)}
                          />
                          <MdDelete
                            className="text-xl cursor-pointer text-red-500"
      ...
      data.onlineSlots.length === 0 ? null : (
        <button
          ...
          style={{ backgroundColor: "#49A6AF" }}
          onClick={saveSlotAndPrice}
          className="w-full  text-white py-2 px-4 rounded-md transition-all mt-4"
        >
          Submit
        </button>
      )}
